...
class CameraHandEyeCalibrator:
    ...
    def calculate_hand_eye_circle_fit(self):
        ...
        # 步骤 1: SVD拟合平面，找到旋转轴方向 (平面法向量)
        points_centroid = points.mean(axis=0)
        _, _, vh = np.linalg.svd(points - points_centroid)
        z_axis_base = vh[2, :]
        if z_axis_base[2] > 0: z_axis_base = -z_axis_base
        print(f"步骤1: 旋转轴在相机坐标系下的方向 (n):\n{np.round(z_axis_base, 4)}")
        ...
        # 步骤 3: 【核心修正】将圆心投影到它所在的旋转平面上
        t_camera_base = (circle_center_calculated - np.dot(circle_center_calculated - points_centroid, z_axis_base) * z_axis_base).reshape(3, 1)
        ...
        x_axis_base = x_axis_base_raw - np.dot(x_axis_base_raw, z_axis_base) * z_axis_base
        x_axis_base /= np.linalg.norm(x_axis_base)
        y_axis_base = np.cross(z_axis_base, x_axis_base)

        # 步骤 5: 构建从基座到相机的变换矩阵 base_H_camera
        R_base_camera = np.stack([x_axis_base, y_axis_base, z_axis_base], axis=0) # R_cam_in_base
        t_base_camera = -R_base_camera @ t_camera_base
        base_H_camera = np.identity(4)
        base_H_camera[:3, :3] = R_base_camera
        base_H_camera[:3, 3] = t_base_camera.flatten()

        print("\n" + "="*50)
        print("【结果 A: 最佳拟合解 (Best Fit Solution)】")
        print("这是最符合您采集数据的数学最优解。")
        self.process_and_save_results(base_H_camera, "hand_eye_calibration_result_best_fit.npz")
        
        # --- 步骤 6 (新增): 基于先验知识的Z轴修正 ---
        print("\n" + "="*50)
        print("【结果 B: Z轴约束解 (Z-Constrained Solution)】")
        print("此解强制相机与基座在用户坐标系下的Z值(高度)为0。")
        base_H_camera_constrained = base_H_camera.copy()
        
        # 在计算出用户坐标系下的变换后，再进行约束
        # 为了避免重复代码，我们在辅助函数中完成
        self.process_and_save_results(base_H_camera_constrained, "hand_eye_calibration_result_z_constrained.npz", constrain_z=True)
        print("="*50)

    def process_and_save_results(self, base_H_camera, filename, constrain_z=False):
        """辅助函数，用于处理坐标系转换、打印和保存结果"""
        # 标准相机(C): +Z向前, +X向右, +Y向下
        # 用户机器人(U): +X向前(深度), +Z向上, +Y向左
        R_U_in_C = np.array([[0, -1, 0], [0, 0, -1], [1, 0, 0]])
        T_C_to_U_coord_transform = np.identity(4)
        T_C_to_U_coord_transform[:3, :3] = R_U_in_C.T

        base_H_user_coord = base_H_camera @ T_C_to_U_coord_transform

        if constrain_z:
            # 在用户坐标系下，将平移的Z分量强制为0
            base_H_user_coord[2, 3] = 0.0

        print("变换矩阵 (相机相对于机械臂基座, 用户定义坐标系):")
        print("说明: +X=向前(深度), +Z=向上, +Y=向左")
        print(np.round(base_H_user_coord, 4))

        np.savez(filename, 
                 base_H_robot_coord=base_H_user_coord, 
                 base_H_camera_coord=base_H_camera)
