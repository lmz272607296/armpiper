...
class HardwarePlayer:
    ...
    def game(self):
        ...
        #-------------------------------------------
        #下面是读取与初始化逻辑
        data = np.load(os.path.join(self.home_path,"colcon_ws/src/open_manipulator/leapsim/leapsim/cache/arm_model_trajectory_2.npy"))
        ...
        for i in range(0, 15, 6):
            ...
            arm.command_joint_position(data[i:i+6, :7], 0.6)

            time.sleep(3)  # 等待每个动作完成
            rclpy.spin_once(leap, timeout_sec=0.1)
            arm.recover(data[i:i+6, :7], leap.raw_arm_positions, 1)
