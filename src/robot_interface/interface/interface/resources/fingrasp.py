...
import torch 
from interface.utils.reformat import omegaconf_to_dict, print_dict
# from interface.utils.utils import set_np_formatting, set_seed, get_current_commit_hash
from interface.utils.no_isaac_rlgames_utils import RLGPUEnv, RLGPUAlgoObserver
...
from interface.learning import amp_players
from interface.learning import amp_models
from interface.learning import amp_network_builder
import numpy as np
...
class HardwarePlayer(object):
    ...
    def deploy(self):
        ...
        #数据说明
        # leap.raw_positions: 形状为 (1, 16)，来自 /hand_joint_states
        # arm.raw_positions: 形状为 (1, 6)，来自 /joint_states_single
        # commands: 形状为 (1, 21)，前5维为策略臂命令，后16维为手部状态
        # history_command: 形状为 (1, 6)，机械臂命令
        # obses: 形状为(1, 21)，由6维臂状态去掉固定关节后拼接16维手状态
        ...
        # 误差回调
            rclpy.spin_once(leap, timeout_sec=0.1)
            arm.command_joint_position(init_arm, 1)
            time.sleep(1)
        ...
        # 状态稳定，开始推理
            rclpy.spin_once(leap, timeout_sec=0.1)
            rclpy.spin_once(arm, timeout_sec=0.1)
            obses= self.order(np.concatenate((arm.raw_positions, leap.raw_positions), axis=1))
            print("check\n")
            history_command= arm.raw_positions


        else:
            obses= np.zeros(21)
            history_command= np.zeros((1, 6))
        ...
        while True:
            ...
            if "disable_actions" not in self.config["task"]["env"]:
                ...
                arm_commands = np.concatenate((history_command, send_commands[:, :6]), axis=0)
                if rclpy.ok() and not test_mode:
                    arm.command_joint_position(arm_commands,1)
                    time.sleep(2)
            ...
            # 历史保存
                    ...
                    obses = commands
                    history_command = arm_commands[-1, :]
                    history_command = np.reshape(history_command, (1, 6))   
    ...
    def reorder(self,data):
        # 从21维策略数据生成22维硬件数据：6维臂 + 16维手。
        rows, cols = data.shape

        if data.shape[1] < 21:
            print("错误：数组的列数少于21，无法执行操作。")
        else:
            fixed_joint = np.zeros((rows, 1), dtype=data.dtype)
            arm_data = np.concatenate((data[:, :3], fixed_joint, data[:, 3:5]), axis=1)
            hand_data = data[:, 5:]
            new_array = np.concatenate((arm_data, hand_data), axis=1)
        return new_array
    
    def order(self,data):
        # 从22维硬件数据提取21维策略观测：去掉6维臂中的固定第4关节。
        rows, cols = data.shape

        if data.shape[1] < 22:
            print("错误：数组的列数少于22，无法执行操作。")
        else:
            part1 = data[:, 0:1]
            part2 = data[:, 1:2]
            part3 = data[:, 2:3]
