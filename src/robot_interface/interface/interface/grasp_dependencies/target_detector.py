...
class TargetDetector(Node):
    ...
    def _normalize_object_type(self, object_type):
        normalized = str(object_type).strip().upper()
        if normalized in ['APPLE', 'ORANGE', 'CUBE']:
            return 'FRUIT'
        return normalized
    ...
    def _listener_callback(self, msg):
        ...
        for target in msg.targets:
                    ...
                    # 检查1：目标类型是否在我们想要的列表中
                    if normalized_object_type in self.target_types_to_find:
                        for point in target.points:
                            if point.type == "center_3d" and point.point:
                                ...
                                if self._is_coord_valid(position_3d):
                                    ...
                                    # 保存找到的目标类型和坐标
                                    self.found_type = normalized_object_type
    ...
    def get_positions(self):
        target_object_list = ['TV', 'FRUIT', 'BOTTLE']
        ...
        elif found_object_type.upper() == 'FRUIT' or found_object_type.upper() == 'TV':
